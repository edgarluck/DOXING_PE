...
def consultar_dni(dni):
    response = requests.get(f'https://bb5b-132-191-2-29.ngrok-free.app/api/personas?dni={dni}')
...
def consultar_nombres(nombres, apellido_paterno, apellido_materno):
    response = requests.get(f'https://bb5b-132-191-2-29.ngrok-free.app/api/personas?nombres={nombres}&apellidoPaterno={apellido_paterno}&apellidoMaterno={apellido_materno}')
    if response.status_code == 200:
        print(response.text)
    else:
        print("Error:", response.text)
